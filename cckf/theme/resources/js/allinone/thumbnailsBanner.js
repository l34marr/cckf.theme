...
(function(f) {
    function L(a, b) {
        ...
        var h = f(a.currentImg.attr("data-text-id")).children(),
            d = 0;
        currentText_arr = [];
        h.each(function() {
            currentText_arr[d] = f(this);
            var g = currentText_arr[d].attr("data-initial-left"),
            ...
            setTimeout(function() {
                ...
                var g = k.attr("data-final-left"),
                    h = k.attr("data-final-top");
                b.responsive && (g = parseInt(g / (b.origWidth / b.width), 10), h = parseInt(h / (b.origWidth / b.width), 10));
                var e = 1;
    ...
    function N(a, b, h) {
        ...
        for (var g = !0, t = 0; t < b.numberOfStripes; t++) t == b.numberOfStripes - 1 && (F = a.width() - B * (b.numberOfStripes - 1)), !b.responsive || -1 == d || -1 != d && 9 <= d ? -1 != z.indexOf("ipad") || -1 != z.indexOf("iphone") || -1 != z.indexOf("ipod") || -1 != z.indexOf("webos") ? g = !1 : a.append(f('<div class="stripe"></div>').css({
            opacity: "0",
            left: B * t + "px",
            width: F + "px",
            height: "0px",
            background: 'url("' + h.current_imgInside.attr("src") + '") ' + -1 * t * B + "px 0%"
        })) : g = !1, g || (mleft = -1 * B * t, a.append(f('<div class="stripe"><img src="' + h.current_imgInside.attr("src") + '" width="' + b.width + '" height="' + b.height + '" style="margin-left:' + mleft + 'px;"></div>').css({
            opacity: "0",
            left: B * t + "px",
            width: F + "px",
            height: "0px"
        })));
        b.responsive && g && (-1 == d || -1 != d && 9 <= d) && f(".stripe", a).css({
    ...
    function K(a, b, f, d, g) {
        ...
        b == d.numberOfRows - 1 && f == d.numberOfColumns - 1 ? setTimeout(function() {
            a.animate({
                opacity: "1.0",
                width: t,
                height: k
    ...
    function O(a, b, f, d, g, t, k) {
        C = b.css("left").substr(0, b.css("left").lastIndexOf("px"));
        0 > C ? f.hasClass("carouselLeftNavDisabled") && f.removeClass("carouselLeftNavDisabled") : f.addClass("carouselLeftNavDisabled");
    ...
    function D(a, b, h, d, g, t, k, n, w, e, m, r) {
        var s = !0;
        if (!e.loop && b.current_img_no + a >= m || !e.loop && 0 > b.current_img_no + a) s = !1;
        if (s) {
            f(".newFS", d).contents().unwrap();
            ...
            f(b.currentImg.attr("data-text-id")).css("display", "none");
            f(g[b.current_img_no]).removeClass("thumbsHolder_ThumbON");
            e.randomizeImages && !b.bottomNavClicked ? (a = Math.floor(Math.random() * m), b.current_img_no = b.current_img_no === a ? Math.floor(Math.random() * m) : a) : b.current_img_no = b.current_img_no + a >= m ? 0 : 0 > b.current_img_no + a ? m - 1 : b.current_img_no + a;
            b.bottomNavClicked = !1;
            f(g[b.current_img_no]).addClass("thumbsHolder_ThumbON");
            C = k.css("left").substr(0, k.css("left").lastIndexOf("px"));
            0 === b.current_img_no || b.current_img_no === m - 1 ? G(0, k, n, w, e, m, r, b) : G(1001, k, n, w, e, m, r, b);
            b.currentImg = f(t[b.current_img_no]);
            b.current_imgInside = b.currentImg.find("img:first");
            b.currentImg.attr("data-transition") ? (h = b.currentImg.attr("data-transition"), "random" == h && (h = I[Math.floor(Math.random() * I.length)])) : h = "random" != e.defaultEffect ? e.defaultEffect : I[Math.floor(Math.random() * I.length)];
            b.effectIsRunning = !0;
            if ("fade" == h || "slideFromLeft" == h || "slideFromRight" == h || "slideFromTop" == h || "slideFromBottom" == h) N(d, e, b), g = f(".stripe:first", d), "fade" == h && (g.css({
                top: "0px",
                height: "100%",
                width: d.width() + "px"
            }), g.animate({
                opacity: "1.0"
            }, 2E3 * e.effectDuration, "", function() {
            ...
            0 <= h.indexOf("Stripes") && (N(d, e, b), g = 0 <= h.indexOf("Reverse") ? f(".stripe", d).myReverse() : f(".stripe", d), u = 100, i = 0, g.each(function() {
                var a = f(this);
                ...
                if ("leftRightFadingStripes" == h || "leftRightFadingReverseStripes" == h) {
                    var b = B;
                    if ("leftRightFadingStripes" == h && i == e.numberOfStripes - 1 || "leftRightFadingReverseStripes" == h && 0 == i) b = F;
                    i == e.numberOfStripes - 1 ? setTimeout(function() {
                        a.animate({
                            width: b + "px",
                            opacity: "1.0"
                        }, 800 * e.effectDuration, "", function() {
                            d.trigger("effectComplete")
                        })
                    }, u) : setTimeout(function() {
                        a.animate({
                            width: b + "px",
                            opacity: "1.0"
                        }, 800 * e.effectDuration)
                    }, u)
                } else i == e.numberOfStripes - 1 ? setTimeout(function() {
                    a.animate({
                        height: "100%",
                        opacity: "1.0"
                    }, 1E3 * e.effectDuration, "", function() {
            ...
            if (0 <= h.indexOf("Blocks")) {
                ...
                f(".stripe", d).remove();
                f(".block", d).remove();
                t = Math.round(e.width / e.numberOfColumns);
                k = Math.round(e.height / e.numberOfRows);
                n = t;
                w = k;
                r = m = 0;
                a = !0;
                for (s = 0; s < e.numberOfRows; s++)
                    for (var J = 0; J < e.numberOfColumns; J++) m = t * J, r = k * s, n = J == e.numberOfColumns - 1 ? e.width - t * (e.numberOfColumns - 1) : t, w = s == e.numberOfRows - 1 ? e.height - k * (e.numberOfRows - 1) : k, !e.responsive || -1 == g || -1 != g && 9 <= g ? -1 != z.indexOf("ipad") || -1 != z.indexOf("iphone") || -1 != z.indexOf("ipod") || -1 != z.indexOf("webos") ? a = !1 : d.append(f('<div class="block"></div>').css({
                        opacity: "0",
                        left: m + "px",
                        top: r + "px",
                        width: n + "px",
                    ...
                    })) : a = !1, a || (mleft = -1 * m, mtop = -1 * r, d.append(f('<div class="block"><img src="' + b.current_imgInside.attr("src") + '" width="' + e.width + '" height="' + e.height + '" style="margin-left:' + mleft + "px; margin-top:" + mtop + 'px;"></div>').css({
                        opacity: "0",
                        left: m + "px",
                        top: r + "px",
                        width: n + "px",
                ...
                e.responsive && a && (-1 == g || -1 != g && 9 <= g) && f(".block", d).css({
                    "-webkit-background-size": e.width + "px " + e.height + "px",
                    "-moz-background-size": e.width + "px " + e.height + "px",
                    "-o-background-size": e.width + "px " + e.height + "px",
                    "-ms-background-size": e.width + "px " + e.height + "px",
                    "background-size": e.width + "px " + e.height + "px"
                });
                if (0 <= h.indexOf("Reverse")) var p = f(".block", d).myReverse();
                ...
                if ("randomBlocks" == h) {
                    i = 0;
                    var y = e.numberOfRows * e.numberOfColumns;
                    p.each(function() {
                        var a = f(this),
                            b = a.width(),
                            c = a.height();
                        ...
                        i == y - 1 ? setTimeout(function() {
                            a.animate({
                                ...
                                width: b,
                                height: c
                            }, 1E3 * e.effectDuration / 3, "", function() {
                                d.trigger("effectComplete")
                            })
                        }, u) : setTimeout(function() {
                            a.animate({
                    ...
                    p.each(function() {
                        x[l][v] = f(this);
                        v++;
                        v == e.numberOfColumns && (l++, v = 0, x[l] = [])
                    });
                    v = l = 0;
                    u = 100;
                    p = f(x[l][v]);
                    for (K(p, 0, 0, e, d); l < e.numberOfRows - 1 || v < e.numberOfColumns - 1;) {
                        l < e.numberOfRows - 1 && l++;
                        v < e.numberOfColumns - 1 && v++;
                        i = l;
                        v < l && e.numberOfRows > e.numberOfColumns && (i = l - v);
                        j = 0;
                        for (l < v && e.numberOfRows < e.numberOfColumns && (j = v - l); 0 <= i && j <= v;) p = f(x[i--][j++]), K(p, i, j, e, d)
                    }
                    u = e.numberOfRows < e.numberOfColumns ? u - (e.numberOfRows - 1) * H : u - (e.numberOfColumns - 1) * H;
                    limit_i = 0;
                    for (limit_j = v - l; limit_i < l && limit_j < v;) {
                        i = l + 1;
                        for (j = limit_j; i > limit_i && j < v;) i -= 1, j += 1, p = f(x[i][j]), K(p, i, j, e, d);
                        limit_i++;
                        limit_j++
                    }
                }
    ...
    function P(a, b, h, d, g, t, k, n, w, e, m) {
        "opportune" != b.skin && (m.css({
            ...
            width: parseInt(b.origThumbW / (b.origWidthThumbsHolderVisibleWrapper / e.width()), 10) + "px",
            height: parseInt(b.origThumbH / (b.origWidthThumbsHolderVisibleWrapper / e.width()), 10) + "px"
        }), b.numberOfThumbsPerScreen >= h && e.css("left", parseInt((m.width() - (w.width() + a.thumbMarginLeft) * h) / 2, 10) + parseInt(a.thumbMarginLeft / 2, 10) + "px"), h = f(".thumbsHolder_ThumbOFF", d).find("img:first"), 0 < b.thumbsReflection && h.unreflect(), h.css({
            width: g.width() + "px",
            height: g.height() + "px",
    ...
    f.fn.allinone_thumbnailsBanner = function(a) {
        a = f.extend({}, f.fn.allinone_thumbnailsBanner.defaults, a);
        return this.each(function() {
            ...
            b.css("display", "block");
            var h = f("<div></div>").addClass("allinone_thumbnailsBanner").addClass(a.skin),
                d = f('<div class="bannerControls"> <div class="leftNav"></div> <div class="rightNav"></div> </div> <div class="contentHolderVisibleWrapper"><div class="contentHolder"></div></div> <div class="thumbsHolderWrapper"><div class="thumbsHolderVisibleWrapper"><div class="thumbsHolder"></div></div></div> <canvas class="mycanvas"></canvas>');
            b.wrap(h);
            b.after(d);
            a.showAllControllers || t.css("display", "none");
            var g = b.parent(".allinone_thumbnailsBanner"),
                t = f(".bannerControls", g),
                k = f(".leftNav", g),
                n = f(".rightNav", g);
            k.css("display", "none");
            n.css("display", "none");
            a.showNavArrows && a.showOnInitNavArrows && (k.css("display", "block"), n.css("display", "block"));
            var w = f(".thumbsHolderWrapper", g),
                e = f(".thumbsHolderVisibleWrapper", g),
                m = f(".thumbsHolder", g),
                r, s;
            r = f('<div class="carouselLeftNav"></div>');
            s = f('<div class="carouselRightNav"></div>');
            w.append(r);
            w.append(s);
            s.css("right", "0");
            ...
            y.each(function() {
                ...
                if (c.currentImg.is("li")) {
                    ...
                    p++;
                    var b = f(y[p - 1]).attr("data-bottom-thumb");
                    l = f('<div class="thumbsHolder_ThumbOFF" rel="' + (p - 1) + '"><img src="' + b + '"></div>');
                    m.append(l);
            ...
            e.css("width", e.width() - r.width() - s.width());
            e.css("left", r.width());
            ...
            d = f(".thumbsHolder_ThumbOFF", g).find("img:first");
            ...
            a.numberOfThumbsPerScreen >= p && (s.addClass("carouselRightNavDisabled"), r.css("display", "none"), s.css("display", "none"), e.css("left", parseInt((w.width() - (l.width() + c.thumbMarginLeft) * p) / 2, 10) + "px"));
            w.css("top", a.height + "px");
            d = f(".thumbsHolder_ThumbOFF", g).find("img:first");
            d.css("margin-top", v + "px");
            a.origthumbsHolder_MarginTop = v;
            var x = f(".thumbsHolder_ThumbOFF", g);
            P(c, a, p, g, x, m, r, s, l, e, w);
            ...
            if (c.current_img_no >= a.numberOfThumbsPerScreen)
                for (q = 0; q < Math.floor(c.current_img_no / a.numberOfThumbsPerScreen); q++) G(-1, m, r, s, a, p, l, c);
            c.currentImg = f(y[c.current_img_no]);
            c.current_imgInside = c.currentImg.find("img:first"); - 1 != z.indexOf("ipad") || -1 != z.indexOf("iphone") || -1 != z.indexOf("ipod") || -1 != z.indexOf("webos") || -1 != h && 7 >= h ? g.append('<img id="curBgImgIos" src="' + c.current_imgInside.attr("src") + '" width="' + a.width + '" height="' + a.height + '">') : (g.css("background", 'url("' + c.current_imgInside.attr("src") + '") no-repeat'), a.responsive && (-1 == h || -1 != h && 9 <= h ? g.css({
            ...
            a.enableTouchScreen && (h = Math.floor(1E5 * Math.random()), g.wrap('<div id="bannerWithThumbnailsParent_' + h + '" style="position:relative;" />'), f("#bannerWithThumbnailsParent_" + h).width(a.width + 1), f("#bannerWithThumbnailsParent_" + h).height(a.height), g.css({
                cursor: "url(" + a.absUrl + "++theme++cckf.theme/img/allinone/skin/hand.cur),url(" + a.absUrl + "++theme++cckf.theme/img/allinone/skin/hand.cur),move",
                left: "0px",
                position: "absolute"
                ...
                stop: function() {
                    c.effectIsRunning || (finalLeft = f(this).css("left"), direction = 1, origLeft < finalLeft && (direction = -1), D(direction, c, u, g, x, y, m, r, s, a, p, l));
                    0 > rightVal && (n.css("right", rightVal + "px"), n.css("visibility", "visible"), k.css("visibility", "visible"));
                    f(this).css("left", "0px")
                }
            }));
            ...
            g.bind("effectComplete", function() {
                var e = E();
                c.effectIsRunning = !1; - 1 != z.indexOf("ipad") || -1 != z.indexOf("iphone") || -1 != z.indexOf("ipod") || -1 != z.indexOf("webos") || -1 != e && 7 >= e ? (f("#curBgImgIos", g).attr("src", c.current_imgInside.attr("src")), f("#curBgImgIos", g).width(a.width), f("#curBgImgIos", g).height(a.height)) : (g.css("background", 'url("' + c.current_imgInside.attr("src") + '") no-repeat'), a.responsive && (-1 == e || -1 != e && 9 <= e ? g.css({
                    "-webkit-background-size": a.width + "px " + a.height + "px",
                    "-moz-background-size": a.width + "px " + a.height + "px",
                    "-o-background-size": a.width + "px " + a.height + "px",
                    "-ms-background-size": a.width + "px " + a.height + "px",
            ...
            g.click(function() {
                if (void 0 != f(y[c.current_img_no]).attr("data-link") && "" != f(y[c.current_img_no]).attr("data-link") && !c.effectIsRunning && !A) {
                    var b = a.target;
                    void 0 != f(y[c.current_img_no]).attr("data-target") && "" != f(y[c.current_img_no]).attr("data-target") && (b = f(y[c.current_img_no]).attr("data-target"));
                    "_blank" == b ? window.open(f(y[c.current_img_no]).attr("data-link")) : window.location = f(y[c.current_img_no]).attr("data-link")
                }
            });
            ...
            f(window).resize(function() {
                ...
                a.responsive && doResizeNow && (!1 !== B && clearTimeout(B), B = setTimeout(function() {
                    ...
                    d.responsiveRelativeToBrowser && (responsiveWidth = f(window).width(), responsiveHeight = f(window).height());
                    ...
                    if (d.origWidth != responsiveWidth || d.width100Proc) {
                        ...
                        g.width(d.width);
                        g.height(d.height); - 1 != z.indexOf("ipad") || -1 != z.indexOf("iphone") || -1 != z.indexOf("ipod") || -1 != z.indexOf("webos") || -1 != A && 7 >= A ? (f("#curBgImgIos", g).attr("src", c.current_imgInside.attr("src")), f("#curBgImgIos", g).width(d.width), f("#curBgImgIos", g).height(d.height)) : -1 == A || -1 != A && 9 <= A ? g.css({
                        ...
                        for (i = 0; i < h; i++) f(f(y[i]).attr("data-text-id")).css("width", b.width() + "px");
                        P(c, d, h, g, v, m, r, s, B, e, w);
                        clearTimeout(c.timeoutID);
                        D(1, c, u, g, v, y, m, r, s, d, h, B)
                    }
                }, 300))
            });
            x = f(".thumbsHolder_ThumbOFF", g);
            x.mousedown(function() {
                A = !0;
                if (!c.effectIsRunning) {
                    var b = f(this).attr("rel");
                    f(x[c.current_img_no]).removeClass("thumbsHolder_ThumbON");
            ...
            f(x[c.current_img_no]).addClass("thumbsHolder_ThumbON");
    ...
    f.fn.allinone_thumbnailsBanner.defaults = {
        skin: "cool",
        width: 1920,
        height: 620,
        width100Proc: !1,
